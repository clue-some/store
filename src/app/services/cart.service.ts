...
export class CartService {
  ...
  addToCart (item: CartItem): void {
    ...
    if (itemInCart) {
      itemInCart.quantity += 1 // does this reference items?
    } else {
      items.push(item);
    }
  ...
  removeQuantity(item: CartItem): void {
    ...
    let mappedItems = this.cart.value.items.map(
      (_item) => {
        if (_item.id === item.id) {
    ...
    if (itemForRemoval) mappedItems = this.removeFromCart(itemForRemoval, false);
